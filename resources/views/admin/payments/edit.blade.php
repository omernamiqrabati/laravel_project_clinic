...
<div class="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-md">
    <h2 class="text-2xl font-semibold text-blue-800 mb-6 border-b pb-2">🦷 Edit Payment</h2>
    ...
    <form action="{{ route('admin.payments.update', $payment['payment_id']) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="invoice_id" id="invoice_id"
                ...
                @foreach($invoices as $invoice)
                    <option value="{{ $invoice['invoice_id'] }}" {{ old('invoice_id', $payment['invoice_id']) == $invoice['invoice_id'] ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="date_of_payment" id="date_of_payment"
                    ...
                    value="{{ old('date_of_payment', $payment['date_of_payment']) }}" required>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <div class="mt-1 relative rounded-lg shadow-sm">
                    ...
                    <input type="number" name="amount_paid" id="amount_paid" step="0.01" min="0"
                        ...
                        placeholder="0.00" value="{{ old('amount_paid', $payment['amount_paid']) }}" required>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="payment_reference" id="payment_reference"
                    class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Transaction ID, Check #, etc." value="{{ old('payment_reference', $payment['payment_reference']) }}">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="received_by" id="received_by"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                placeholder="Staff member who received payment" value="{{ old('received_by', $payment['received_by']) }}" required>
        </div>
        ...
        <div class="flex justify-end pt-4">
            <button type="submit"
                ...
                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4 7v10c0 2.21 1.79 4 4 4h8c2.21 0 4-1.79 4-4V7M4 7l16 0M4 7l1-3h14l1 3M9 5v2m6-2v2"></path>
                </svg>
                Update Payment
            </button>
        </div>
